...
- **La decisión** (→ [[adr/0021-fk-compuestas-multi-tenant]]): FK compuesta `(clinicId, appointmentId)` → `Appointment(clinicId, id)`, con `@@unique([clinicId, id])` en Appointment. Un par cruzado deja de ser un bug que hay que recordar evitar y pasa a ser un INSERT que Postgres rechaza.
