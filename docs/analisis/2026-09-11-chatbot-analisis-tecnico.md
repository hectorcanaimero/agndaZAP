...
- [ ] **S2** Paciente que solo manda notas de voz (decidido 2026-09-11, delegado por el owner): el primer audio recibe el aviso "solo leo texto"; si llega un **segundo mensaje sin texto sin un texto en medio** (ventana 24 h), la conversación pasa a `NEEDS_HUMAN` y el bot responde "Te paso con una persona del equipo para escucharte". Recepción ve el `[audio]` en la bandeja. Implementar en `webhook.controller.ts` (mismo camino de PR #43). Ver `docs/notas/2026-09-11-waha-mensajes-sin-texto.md`. Sustituido a futuro por M10.
