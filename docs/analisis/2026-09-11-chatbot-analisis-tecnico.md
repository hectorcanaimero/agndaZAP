...
- [x] **B6** `AI_DISCLOSURE` una vez por conversación cada 24 h; siempre en el primer saludo (**rama `feat/bot-disclosure-24h`**). Sin columna nueva: se mira si hay un `Message OUT` reciente cuyo cuerpo CONTENGA el aviso — contar tráfico OUT a secas dejaba sin aviso a quien manda un adjunto antes de saludar, porque el webhook y los processors también escriben OUT. Encuadre de compliance aprobado por el owner en [[adr/0004-pii-y-compliance]] §7.1, donde queda anotada la deuda de las ramas que no son el saludo.
