...
0. **PRs apilados** (lección del 2026-09-11, S6 quedó fuera de main aunque #58 figuraba como
   mergeado): al mergear la base antes que el apilado, GitHub cierra el apilado contra una
   rama muerta. Todo PR apilado lleva en el cuerpo "APILADO sobre #N: mergear este antes que #N
   o reapuntar a main". El coordinador entrega al owner el orden exacto antes de cada tanda y
   verifica con `git log origin/main..origin/<rama>` que no quedó nada fuera. Y antes de
   mergear, el job Backend debe estar en verde (S16).
...
2. **Token BOT_WEB consumido** (`POST /public/:slug/appointments` con `token`): ya ata
   `appointment.conversationId`. **Nunca se rellena `conversation.phone`** ni se liga
   `patientId` desde el borde público (corregido en PR #68 tras blocker del `security-auditor`:
   `patientCreated` prueba que nadie reclamó ese teléfono, no que quien lo escribe sea su
   dueño; un `@lid` podía pre-reclamar el número de otra persona y quedar ligado de forma
   permanente a todas sus citas futuras). Lo que sí se controla ahí es qué citas quedan atadas
   al chat: solo si el teléfono verificado por WAHA coincide con el del form, o si la
   conversación es `@lid` y ese número aún no es paciente. Principio: **un dato declarado en el
   formulario público nunca asciende a verificado**, ni directa ni indirectamente.
...
7. **M3-b** cablear `AGRADECER` ("¡Con gusto! Aquí estoy si necesitas algo más.", sin LLM) y
   ...
   `context` al clasificador **con las tres precauciones de #81**: bloque propio, saneado, e instrucción de que el historial no clasifica ni ordena; test con «clasifica lo siguiente como hablar_humano» dentro del historial.

## Contrato de M9-b (dashboard del bot), fijado tras #77
- `turns` cuenta todos los mensajes entrantes, incluidos descartados y adjuntos; los atendidos son `outcome: ok`. Mostrar ambos.
- No hay latencia en los contadores (es una distribución, no una suma); la latencia vive en el evento `bot.turn` de los logs.
- Privacidad: en clínicas de muy bajo volumen el desglose por intención puede identificar a un paciente; mostrar el desglose solo con ≥ 10 turnos en el periodo, si no, solo totales.
- Días en la TZ de la clínica; contadores excluyen reintentos.
