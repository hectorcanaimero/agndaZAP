...
import { PatientsClient, type PatientListResponse } from './PatientsClient';
...
export default async function PatientsPage({
  ...
  const t = await getTranslations('panel.patients');

  const token = await getTokenFromCookies();
  const res = await fetcher<PatientListResponse>('/api/patients?limit=50', {
    token,
  });
