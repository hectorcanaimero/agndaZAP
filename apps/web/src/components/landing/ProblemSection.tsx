      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:items-center lg:gap-16">
          <div className="relative min-w-0">
            ...
            {/* Callout flotante con la estadística honesta del PRD */}
            <div className="absolute -bottom-6 -right-3 hidden max-w-[220px] rounded-2xl border border-neutral-200 bg-white p-4 shadow-xl sm:block">
              <div className="font-display text-3xl font-semibold leading-none text-brand-700">
                20–30%
              </div>
              <div className="mt-1 text-xs leading-relaxed text-neutral-600">
                de los ingresos que se pierden por no-show y gestión manual (estimado en LATAM).
              </div>
            </div>
          </div>
            ...
            <ul className="mt-8 space-y-6">
              {points.map((k) => (
                <li key={k} className="flex gap-4">
                  <span
                    aria-hidden="true"
                    className="mt-1.5 inline-block h-2 w-2 shrink-0 rounded-full bg-brand-600"
                  />
                  <div className="min-w-0">
